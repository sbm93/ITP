...
class CUSTOM_STRAT_THIRTEEN_VO_SRT(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position: #self.macdhist.macd[0] > self.macdhist.signal[0] and 
            if self.dataclose[0] > self.fpp.r1[-1] and self.sma5[0] > self.sma25[0]:
                ...
                if self.dataclose[0] < self.fpp.s1[-1] and self.sma5[0] < self.sma25[0]:    
