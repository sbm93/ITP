...
class CUSTOM_STRAT_THIRTEEN_V1_SRT(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.highest = bt.indicators.Highest(self.datahigh,period=self.params.period_high)
        self.lowest = bt.indicators.Lowest(self.datalow,period=self.params.period_low)
    ...
    def next(self):
        ...
        if not self.position: #self.macdhist.macd[0] > self.macdhist.signal[0]  #self.dataclose[0] > self.fpp.r1[-1] and self.dataclose[-1] < self.fpp.r1[-2] 
            if self.diff38co[0]==1 and self.datahigh[0] > self.datahigh[-1] and self.datahigh[-1] > self.datahigh[-2] and self.macdhist.macd[0] > self.macdhist.signal[0]:
                        ...
                        print('long entry')
                        print(lentryprice)
            else: # self.dataclose[0] < self.fpp.s1[-1] and self.dataclose[-1] > self.fpp.s1[-2] 
                if self.diff61co[0]==-1 and self.datalow[0] < self.datalow[-1]  and self.datalow[-1] < self.datalow[-2]  and self.macdhist.macd[0] < self.macdhist.signal[0]:    
...
def run_strategy(strategy, variables, 
                 ...
                 issg=False):
    print("run strategy")
    print(period_high, period_low, period_me1, period_me2, period_sig)
    cerebro = bt.Cerebro()
...
print(time_b-time_a)
list = pd.DataFrame(final_results_list, columns = ['period_high', 'period_low', 'period_me1','period_me2','period_sig', 'return', 'pnl', 'drawdown (%)', 'drawdown (abs)'])
list.to_csv("gl_fcpo_d_000013_v1_Single.csv")
