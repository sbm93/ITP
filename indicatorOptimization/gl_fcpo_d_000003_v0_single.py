...
# Create a Stratey
class CUSTOM_STRAT_THREE_VO_SINGLE(bt.Strategy):
    ...
    params = dict(
        ...
        period_aosma9 = period_aosma9,
        period_aosma12 = period_aosma12,
    ...
    def __init__(self):
        ...
        self.sma5 = bt.indicators.SMA((self.datahigh+self.datalow)/2, period=self.params.period_sma5, plot=False)
        self.sma25 = bt.indicators.SMA((self.datahigh+self.datalow)/2, period=self.params.period_sma25, plot=False)        
        ...
        self.ao = bt.indicators.AwesomeOscillator(self.datas[0],fast=self.params.period_fast,slow=self.params.period_slow)
        self.aosma9 = bt.indicators.SMA(self.ao, period=self.params.period_aosma9,plot=False)
        self.aosma12 = bt.indicators.SMA(self.ao, period=self.params.period_aosma12,plot=False)
...
def run_strategy(strategy, variables,
                 ...
                 issg=False):
    print("run strategy")
    print(period_sma5, period_sma25, period_aosma9, period_aosma12)
    cerebro = bt.Cerebro()
...
i = 0
for period_sma5 in np.arange(6, 17, 3):
    for period_sma25 in np.arange(period_sma5+2, 19, 3):
        for period_fast in np.arange(6, 20, 3):  
            for period_slow in np.arange(period_fast+2, 22, 3):
                for period_aosma9 in np.arange(6, 15, 3):  # 6
                    for period_aosma12 in np.arange(6, 15, 3): # 6
                        i = i+1
                        res = run_strategy(CUSTOM_STRAT_THREE_VO_SINGLE, strategy_variable['SG'], 
                                           period_sma5,
                                           period_sma25,
                                           period_fast,
                                           period_slow,
                ...
                #                round_trip_data, drawdown_df = gen_report(returns, positions, transactions, gross_lev, benchmark, strategy_variable['SG'])
                        ...
                        final_results_list.append([period_sma5,period_sma25,period_fast, period_slow, period_aosma9, period_aosma12, rd_ret[rd_ret.columns[0]].iloc[0],round_trip_data['pnl'][round_trip_data['pnl'].columns[0]].iloc[0].round(2), df_per[df_per.columns[0]].iloc[0], df_abs[df_abs.columns[0]].iloc[0] ])
                        print("Number of iteration: ")
                        print(i)

time_b = datetime.datetime.now()
print(time_b-time_a)
list = pd.DataFrame(final_results_list, columns = ["period_sma5","period_sma25","period_fast","period_slow","period_aosma9", "period_aosma12", 'return', 'pnl', 'drawdown (%)', 'drawdown (abs)'])
list.to_csv("gl_fcpo_d_000003_v0_single.csv")
