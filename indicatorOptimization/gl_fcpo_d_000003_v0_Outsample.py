    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.position.size > 0:
                if(self.he[0] > maxhe):
                    maxhe = self.he[0]
                if self.direc.plusDI[0] < self.direc.minusDI[0] : #and self.dataclose[0] > self.dataopen[0] :
                    print("LongPExit")
...
print(time_b-time_a)
list = pd.DataFrame(final_results_list, columns = ["period_direc", "period_he", 'return', 'pnl', 'drawdown (%)', 'drawdown (abs)'])
list.to_csv("gl_fcpo_d_000003_v0_Outsample.csv")
