...
for period_me1 in np.arange(6, 20, 2):  # 6
    for period_me2 in np.arange(6, 20, 2):  # 6
        for period_sig in np.arange(6, 20, 2):  # 6
            for period_band in np.arange(6, 20, 2): # 6
                i = i+1
        ...
        #                round_trip_data, drawdown_df = gen_report(returns, positions, transactions, gross_lev, benchmark, strategy_variable['SG'])
                ...
                final_results_list.append([period_me1, period_me2, period_sig, period_band, rd_ret[rd_ret.columns[0]].iloc[0],round_trip_data['pnl'][round_trip_data['pnl'].columns[0]].iloc[0].round(2), df_per[df_per.columns[0]].iloc[0], df_abs[df_abs.columns[0]].iloc[0] ])
...
list.to_csv("gl_fcpo_d_000007_v0_Single.csv")
