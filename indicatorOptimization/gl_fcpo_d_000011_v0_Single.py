...
period_high = 4
period_low = 4
class CUSTOM_STRAT_ELEVEN_VO_SRT(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position: #self.macdhist.macd[0] > self.macdhist.signal[0] and 
            ...
            and self.dataclose[0] > self.fpp.r1[-1]:
                ...
                and self.dataclose[0] < self.fpp.s1[-1]:    
...
for period_high in np.arange(6, 20, 2):  # 6
    for period_low in np.arange(6, 20, 2): # 6
        i = i+1
        res = run_strategy(CUSTOM_STRAT_ELEVEN_VO_SRT, strategy_variable['SG'], 
                           period_high, 
                           period_low, 
                           issg=False)                        
...
#                round_trip_data, drawdown_df = gen_report(returns, positions, transactions, gross_lev, benchmark, strategy_variable['SG'])
        ...
        final_results_list.append([period_high, period_low, rd_ret[rd_ret.columns[0]].iloc[0],round_trip_data['pnl'][round_trip_data['pnl'].columns[0]].iloc[0].round(2), df_per[df_per.columns[0]].iloc[0], df_abs[df_abs.columns[0]].iloc[0] ])
        print("Number of iteration: ")
        print(i)

time_b = datetime.datetime.now()
print(time_b-time_a)
list = pd.DataFrame(final_results_list, columns = ['period_high', 'period_low', 'return', 'pnl', 'drawdown (%)', 'drawdown (abs)'])
list.to_csv("gl_fcpo_d_000011_v0_Single.csv")
