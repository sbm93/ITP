...
class CUSTOM_STRAT_FOURTEEN_VO_SRT(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position: 
            if self.ha.ha_close[0] > self.ha.ha_open[0] and (self.ha.ha_high[-1] - self.ha.ha_close[-1]) > (self.ha.ha_high[0] - self.ha.ha_close[0])\
            and (self.ha.ha_high[-2] - self.ha.ha_close[-2]) > (self.ha.ha_high[0] - self.ha.ha_close[0]) and self.ha.ha_low[-1] < self.ha.ha_low[-2]\
            and self.ha.ha_low[-1] < self.ha.ha_low[0] :
                ...
                if self.ha.ha_close[0] < self.ha.ha_open[0] and (self.ha.ha_close[-1] - self.ha.ha_open[-1]) > (self.ha.ha_close[0] - self.ha.ha_low[0])\
                and (self.ha.ha_close[-2] - self.ha.ha_low[-2]) > (self.ha.ha_close[0] - self.ha.ha_low[0]) and self.ha.ha_high[-1] > self.ha.ha_high[-2]\
                and self.ha.ha_high[-1] > self.ha.ha_high[0] :
        ...
        for period_me1 in np.arange(3, 20, 3):  # 6
            for period_me2 in np.arange(6, 30, 2): # 6
                for period_sig in np.arange(3, 20, 2):
                    i = i+1
                    res = run_strategy(CUSTOM_STRAT_FOURTEEN_VO_SRT, strategy_variable['SG'], period_sma5, period_sma25, period_me1, period_me2, period_sig, False)                        
            ...
            #                round_trip_data, drawdown_df = gen_report(returns, positions, transactions, gross_lev, benchmark, strategy_variable['SG'])
                    ...
                    final_results_list.append([period_sma5, period_sma25, period_me1, period_me2, period_sig, rd_ret[rd_ret.columns[0]].iloc[0],round_trip_data['pnl'][round_trip_data['pnl'].columns[0]].iloc[0].round(2), df_per[df_per.columns[0]].iloc[0], df_abs[df_abs.columns[0]].iloc[0] ])
