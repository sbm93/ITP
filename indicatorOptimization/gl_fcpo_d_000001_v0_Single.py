...
period_he = 20
class ONE_VO_STRAT_SINGLE(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if self.he[0] > self.he[-1] and self.he[-1] > self.he[-2]  and self.datahigh[0] < self.datahigh[-1] and self.datalow[0] > self.datalow[-1] and self.dataclose[0] > self.dataopen[0]: #and self.he[0] > self.he[-1] 
                ...
                if self.he[0] > self.he[-1] and self.he[-1] > self.he[-2]  and self.datahigh[0] < self.datahigh[-1] and self.datalow[0] > self.datalow[-1] and self.dataclose[0] < self.dataopen[0]:
            ...
            if self.position.size > 0:
                if(self.he[0] > maxhe):
                    maxhe = self.he[0]
                #if self.dataclose[0] < self.sma25[0] and self.he[0] < self.he[-1]: #
                #if self.datahigh[0] < self.datahigh[-1] and self.datalow[0] < self.datalow[-1] and self.dataclose[0] < self.dataopen[0]: #self.he[0] < self.he[-1] and self.dataclose[0] > self.dataopen[0] and self.he[-1] < maxhe :
                if self.he[0] > self.he[-1] and self.he[-1] > self.he[-2]  and self.datahigh[0] < self.datahigh[-1] and self.datalow[0] > self.datalow[-1] and self.dataclose[0] < self.dataopen[0]:
                    print("LongPExit")
            ...
            if self.position.size < 0:
                if(self.he[0] < minhe):
                    minhe = self.he[0]
                #if self.datahigh[0] > self.datahigh[-1] and self.datalow[0] > self.datalow[-1] and self.dataclose[0] > self.dataopen[0] : #self.he[0] > self.he[-1] and self.dataclose[0] < self.dataopen[0] and self.he[-1] > minhe :
                if self.he[0] > self.he[-1] and self.he[-1] > self.he[-2]  and self.datahigh[0] < self.datahigh[-1] and self.datalow[0] > self.datalow[-1] and self.dataclose[0] > self.dataopen[0]:
                    print("ShortPExit")
...
def run_strategy(strategy, variables, 
    ...
    if issg == False:
        cerebro.addstrategy(strategy, period_he=period_he)
    else:
...
i = 0
for period_he in np.arange(6, 50):
    i = i+1
    res = run_strategy(ONE_VO_STRAT_SINGLE, strategy_variable['SG'], 
                       period_he, False)
    ...
    final_results_list.append([period_he, rd_ret[rd_ret.columns[0]].iloc[0],round_trip_data['pnl'][round_trip_data['pnl'].columns[0]].iloc[0].round(2), df_per[df_per.columns[0]].iloc[0], df_abs[df_abs.columns[0]].iloc[0] ])
    print("Number of iteration: ")
    print(i)

time_b = datetime.datetime.now()
print(time_b-time_a)
list = pd.DataFrame(final_results_list, columns = ['period_he', 'return', 'pnl', 'drawdown (%)', 'drawdown (abs)'])
list.to_csv("gl_fcpo_d_000001_v0_Single.csv")
