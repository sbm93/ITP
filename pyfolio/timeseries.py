...
def max_drawdown(returns):
    ...
    return df_cum_rets.sub(cum_max_return).div(cum_max_return).max()
...
def Gross_Profit_Yearly(returns, period=DAILY):
    """Determines the annual returns of a strategy.
    ...
    Gross_Profit_Yearly = (end_value - start_value)
    #annual_return = (1. + total_return) ** (1 / num_years) - 1

    return Gross_Profit_Yearly

def Gross_Profit_Monthly(returns, period=MONTHLY):
    """Determines the annual returns of a strategy.
    ...
    Gross_Profit_Monthly = (end_value - start_value)
    #annual_return = (1. + total_return) ** (1 / num_years) - 1

    return Gross_Profit_Monthly


def Gross_Profit_Weekly(returns, period=WEEKLY):
    """Determines the annual returns of a strategy.
    ...
    Gross_Profit_Weekly = (end_value - start_value)
    #annual_return = (1. + total_return) ** (1 / num_years) - 1

    return Gross_Profit_Weekly
...
def sharpe_ratio(returns, risk_free=0, period=DAILY):
    ...
    return annual_return(returns_risk_adj) / \
        (np.std(returns_risk_adj) * np.sqrt(ANNUALIZATION_FACTORS[period]))
...
def information_ratio(returns, factor_returns):
    ...
    """
    annret = annual_return(returns)    
    factannret = annual_return(factor_returns)    
    ...
    return (annret-factannret) / tracking_error
...
"""
    ...
    #beta, alpha = sp.stats.linregress(returns.values,factor_returns.loc[ret_index].values)[:2]
...
def aggregate_returns(df_daily_rets, convert_to):
    ...
    if convert_to == WEEKLY:
        ...
        print(cumulate_returns)
        a=df_daily_rets.groupby(
...
def get_top_drawdowns_from_pnl(returns, top=10):

    returns = returns.copy()
    df_abs = abs_returns(returns, 1.0)
    running_max = np.maximum.accumulate(df_abs)
    #underwater = df_abs / running_max - 1
    underwater = df_abs - running_max 
...
def gen_drawdown_table_for_absolute(returns, top=10, investment=1.0):
    df_cum = cum_returns(returns, investment)
    ...
    for i, (peak, valley, recovery) in enumerate(drawdown_periods):
        ...
        if isinstance(recovery, float):
            ...
            df_drawdowns.loc[i, 'recovery date'] = (recovery.to_pydatetime()
                                                    .strftime('%Y-%m-%d'))
        df_drawdowns.loc[i, 'net drawdown'] = (df_cum.loc[peak] - df_cum.loc[valley])
