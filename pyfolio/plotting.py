...
def plot_monthly_returns_heatmap(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    monthly_ret_table = monthly_ret_table.sort_index(ascending=True)
    
    ax.axvline(
        100 *
        monthly_ret_table.values.mean(),
        color='steelblue',
        linestyle='--',
        lw=4,
        alpha=0.7)
    (100 * monthly_ret_table.sort_index(ascending=False)
     ).plot(ax=ax, kind='bar', alpha=0.70, **kwargs)
...
def plot_annual_returns(returns, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    (100 * ann_ret_df.sort_index(ascending=True)
     ).plot(ax=ax, kind='bar', color=[colors_s], alpha=0.70, **kwargs)
    ax.axvline(0.0, color='black', linestyle='-', lw=3)
    ...
    ax.set_title("Annual Returns")
    ax.legend(['mean'])
    return ax, ann_ret_df


def plot_monthly_returns(returns, ax=None, **kwargs):
    """
    Plots a bar graph of returns by year.
    ...
    **kwargs, optional
    ...
    ann_ret_df = ann_ret_df.reset_index(level=1, drop=True)
    ...
    ax.axvline(
        ...
        alpha=0.7)
    (100 * ann_ret_df.sort_index(ascending=True)
     ).plot(ax=ax, kind='bar',alpha=0.70,  
         color=[colors_s], **kwargs)
    ax.axvline(0.0, color='black', linestyle='-', lw=3)
...
def plot_drawdown_periods(returns, top=10, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    y_axis_formatter = FuncFormatter(utils.round_two_dec_places)
#    ax.yaxis.set_major_formatter(FuncFormatter(y_axis_formatter))
...
def plot_rolling_returns(returns,
    ...
    **kwargs, optional
    ...
    cum_rets = timeseries.cum_returns(returns, investment)
    ...
    if factor_returns is not None:
        cum_factor_returns = timeseries.cum_returns(
            factor_returns[cum_rets.index], investment)
        previousValue = 0
        for i, v in cum_factor_returns.items():
            if np.isnan(v):
                cum_factor_returns[i] = previousValue
            previousValue = cum_factor_returns[i]
...
def plot_return_quantiles(returns, live_start_date=None, ax=None, **kwargs):
    ...
    **kwargs, optional
    ...
    data = {'is_returns': is_returns, 'is_weekly': is_weekly, 'is_monthly': is_monthly}
    return ax, data
...
def show_worst_drawdown_periods(returns, top=5, investment=1.0):
    ...
    drawdown_df = {}
    drawdown_df['percentage'] = timeseries.gen_drawdown_table(returns, top=top)
    drawdown_df['absolute'] = timeseries.gen_drawdown_table_for_absolute(returns, top=top, investment=investment)
