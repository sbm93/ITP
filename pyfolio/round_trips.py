...
SUMMARY_STATS = OrderedDict(
    [('Total number of round trades', 'count'),
     ('Number of winning trade', lambda x: x[x>=0].count()),
     ('Number of losing trade', lambda x: x[x<0].count()),
     ('Winning trade in (%)', lambda x: x[x>=0].count()/x.count()*100),
     ('Largest winning trade ($)', 'max'),
     ('Largest losing trade ($)', 'min')
     ])

RETURN_STATS = OrderedDict(
    [('Returns all round trades (%)', lambda x: x.sum()),
     ('Avg returns all round trades (%)', lambda x: x.sum()),
     ('Largest winning trade', 'max'),
     ('Largest losing trade', 'min')
     ])
...
def agg_all_long_short(round_trips, col, stats_dict):
    ...
    stats_long_short = (round_trips
                        ...
                        .agg(stats_dict)
                        .T
                        .rename_axis({True: 'Long trades',
                                      False: 'Short trades'},
                                     axis='columns'))
    
# =============================================================================
#     stats_all_percentage = (round_trips
#                             .)
# =============================================================================
            ...
            if (len(price_stack) == 0) or \
               (copysign(1, price_stack[-1]) == copysign(1, t.amount)):
                ...
                roundtrips.append({'pnl': pnl,
                                   ...
                                   'close_price': indiv_prices[0]
                                   })

    roundtrips = pd.DataFrame(roundtrips)
    roundtrips['duration'] = roundtrips['close_dt'] - roundtrips['open_dt']
#    roundtrips['pnl_shared'] = roundtrips['pnl']
    roundtrips['pnl_shared'] = roundtrips['pnl'] * shares_held
#    roundtrips['pnl_shared'] = (( roundtrips['pnl'] - slippage ) * ( roundtrips['open_price'] + roundtrips['close_price']) ) * shares_held

    roundtrips['shares_held'] = shares_held
    ...
    if portfolio_value is not None:
        ...
        pv['date'] = pv['date'].dt.date

        roundtrips['date'] = roundtrips.close_dt.dt.date

        tmp = roundtrips.join(pv.set_index('date'), on='date')
        
        roundtrips['returns'] = tmp.pnl_shared / tmp.portfolio_value
...
def gen_round_trip_stats(round_trips):
    ...
    stats = {}
    stats['pnl'] = agg_all_long_short(round_trips, 'pnl_shared', PNL_STATS)

    stats['summary'] = agg_all_long_short(round_trips, 'pnl_shared',
                                          SUMMARY_STATS)
										  
    stats['duration'] = agg_all_long_short(round_trips, 'duration',
                                           DURATION_STATS)
    stats['returns'] = agg_all_long_short(round_trips, 'pnl_shared',
                                       RETURN_STATS)
