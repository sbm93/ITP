...
#        print('benchmark_rets is None')
        benchmark_rets = utils.get_symbol_rets('SPY') #,start=live_start_date)
...
#        print('unadjusted_returns is None')
        turnover = txn.get_turnover(positions, transactions,
                                    period=None, average=False)
        unadjusted_returns = returns.copy()
#        print('returns returns returns')
#        print(returns)
        returns = txn.adjust_returns_for_slippage(returns, turnover, slippage)
#        print(returns)
        
    return_tear_sheet_charts = {}
    return_tear_sheet_charts['fig1'], show_perf_stats = create_returns_tear_sheet_cum(
        returns,
    ...
    return_tear_sheet_charts['fig2'], drawdown_df = create_returns_tear_sheet_multi(
        returns,
        live_start_date=live_start_date,
        cone_std=cone_std,
        benchmark_rets=benchmark_rets,
    ...
    return_tear_sheet_charts['fig3'], cob_data = create_returns_tear_sheet_ret(
        returns,
        live_start_date=live_start_date,
        cone_std=cone_std,
        benchmark_rets=benchmark_rets,
    ...
    #time_tear_fig = create_interesting_times_tear_sheet(returns,
    #                                    benchmark_rets=benchmark_rets,
    #                                    set_context=set_context)
    if positions is not None:
#        print('positions is not None')
        position_tear_fig = create_position_tear_sheet(returns, positions,
                                   gross_lev=None,
...
#            print('transactions is not None')
            txn_tear_fig = create_txn_tear_sheet(returns, positions, transactions,
                                  unadjusted_returns=unadjusted_returns,
                                  set_context=set_context)
            if round_trips is not None:
#                print('round_trips is not None')
                trip_tear_fig, round_trip_data,  round_trip_trades = create_round_trip_tear_sheet(
                    returns=returns,
                    positions=positions,
                    ...
                    shares_held=shares_held,
                    slippage=slippage)
#                print("completed round trip")

            if market_data is not None:
#                print('market_data is not None')
                create_capacity_tear_sheet(returns, positions, transactions,
                                           market_data, daily_vol_limit=0.2,
                                           last_n_days=125)

    if bayesian:
#        print('bayesian is not None')
        bayesian_tear_fig = create_bayesian_tear_sheet(returns,
                                   live_start_date=live_start_date,
                                   benchmark_rets=benchmark_rets,
                                   set_context=set_context)
    
    return returns, trip_tear_fig, round_trip_data, drawdown_df, cob_data, return_tear_sheet_charts, round_trip_trades, show_perf_stats
...
def create_returns_tear_sheet(returns, live_start_date=None,
    ...
    set_context : boolean, optional
        If True, set default plotting style context.
    """
#    print('live start date-param')
#    print(live_start_date)
#    benchmark_rets = returns
    if benchmark_rets is None:
#        print('benchmark_rets in not none')
...
#    print(benchmark_rets)
    #benchmark_rets.index.tz = None
    #benchmark_rets.index = pd.to_datetime(benchmark_rets.index).date
...
#    print('\n')
    ...
    show_perf_stats = plotting.show_perf_stats(returns, benchmark_rets,
                             bootstrap=bootstrap,
                             live_start_date=live_start_date)
    #print(101)
    ...
    fig = plt.figure(figsize=(12, vertical_sections * 5))
#    print('vertical_sections')
#    print(vertical_sections)
...
#    data = {'monthly_return': monthly_return, 
#            'annual_return': annual_return, 
#            'monthly_return_dist': monthly_return_dist, 
#            'club_data': club_data}
    
    return fig, show_perf_stats # , drawdown_df, data


@plotting_context
def create_returns_tear_sheet_cum(returns, live_start_date=None,
                              cone_std=(1.0, 1.5, 2.0),
                              benchmark_rets=None,
                              bootstrap=False,
                              return_fig=False,
...
#    print('\n')
    ...
    gs = gridspec.GridSpec(4, 1, wspace=0.5, hspace=0.5)
    
    ax_rolling_returns = plt.subplot(gs[:2, :])
#    print("ax")
#    print(ax_rolling_returns)

    #ax_rolling_returns_vol_match = plt.subplot(gs[2:4, :])
    ax_rolling_equity_curve = plt.subplot(gs[2:4, :])
    ...
    plotting.plot_rolling_returns(
        ...
        cone_std=cone_std,
        investment=investment,
        ax= ax_rolling_equity_curve)
    ax_rolling_equity_curve.set_title(
        'Equity Curve')
...
@plotting_context
def create_returns_tear_sheet_multi(returns, live_start_date=None,
                              cone_std=(1.0, 1.5, 2.0),
                              benchmark_rets=None,
                              bootstrap=False,
                              return_fig=False,
...
def create_returns_tear_sheet_ret(returns, live_start_date=None,
    ...
    i = 0
    ax_monthly_returns = plt.subplot(gs[i, : ])
    i += 1
    ax_annual_returns = plt.subplot(gs[i, : ])
    i += 1
    ax_monthly_dist = plt.subplot(gs[i, : ])
    i += 1
    ax_return_quantiles = plt.subplot(gs[i, :])

    plotting.show_return_range(returns)
    
    #monthly_return_ax, monthly_return = plotting.plot_monthly_returns_heatmap(returns, ax=ax_monthly_heatmap)
    monthly_return_ax, monthly_return = plotting.plot_monthly_returns(returns,ax=ax_monthly_returns)
    annual_return_ax, annual_return = plotting.plot_annual_returns(returns, ax=ax_annual_returns)
    monthly_return_dist_ax, monthly_return_dist = plotting.plot_monthly_returns_dist(returns, ax=ax_monthly_dist)

    plot_return_quantiles_ax, club_data = plotting.plot_return_quantiles(
        returns,
        live_start_date=live_start_date,
...
@plotting_context
def create_round_trip_tear_sheet_chart(returns, pnl, duration):
    fig = plt.figure(figsize=(14, 3 * 6))
    gs = gridspec.GridSpec(3, 1, wspace=0.5, hspace=0.5)
    
    i = 0
    ax_holding_time = plt.subplot(gs[i, :])
    i += 1
    ax_pnl_per_round_trip_dollars = plt.subplot(gs[i, :])
    i += 1
    ax_pnl_per_round_trip_pct = plt.subplot(gs[i, :])

    trade_holding_times = [x.days for x in duration]
    sns.distplot(trade_holding_times, kde=False, ax=ax_holding_time)
    ax_holding_time.set(xlabel='holding time in days',ylabel='')

    sns.distplot(pnl, kde=False, ax=ax_pnl_per_round_trip_dollars)
    ax_pnl_per_round_trip_dollars.set(xlabel='PnL per round-trip trade in $',ylabel='')

    sns.distplot(returns.dropna() * 100, kde=False,
                 ax=ax_pnl_per_round_trip_pct)
    ax_pnl_per_round_trip_pct.set(
        xlabel='Round-trip returns in %',ylabel='')
    gs.tight_layout(fig)
